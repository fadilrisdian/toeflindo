...
/** Group rows by calendar date (first 10 chars of ISO string). */
...
function AllMistakesContent() {
  ...
  useEffect(() => {
    api.get<FilterOptions>('/api/grammar/filter-options').then(setFilterOpts).catch(() => {})
  }, [])
  ...
  function navigate(opts: {
    ...
    router.push(`/practice/grammar/all-mistakes?${params.toString()}`)
  }
    ...
    <>
      ...
      <main style={{ maxWidth: 700, margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: '#9ca3af', marginBottom: '1.5rem' }}>
          ...
          <span>/</span>
          <span style={{ color: '#1f2937', fontWeight: 600 }}>All Mistakes</span>
        </div>

        {/* Header */}
        <div style={{ marginBottom: '1.25rem' }}>
          <h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 700, color: '#1f2937' }}>All Mistakes</h1>
          {data && (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 4 }}>
                  {group.items.map(m => (
                    <div key={m.id} style={{ position: 'relative' }}>
                      <Link href={`/practice/grammar/mistake/${m.id}`} style={{ textDecoration: 'none' }}>
                        <div
                          className="table-card"
                          style={{ padding: '12px 16px', cursor: 'pointer', transition: 'border-color 0.15s, background 0.15s', paddingRight: 110 }}
                      ...
                      {/* Strengthen button — absolute so it doesn't interfere with the card link */}
                      <Link
                        href={`/practice/grammar/remediate/${m.id}`}
                        onClick={e => e.stopPropagation()}
                        style={{
                          position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)',
                          fontSize: '0.68rem', fontWeight: 600, color: '#2a7a7a',
...
export default function AllMistakesPage() {
  return <RequireAuth><AllMistakesContent /></RequireAuth>
}
